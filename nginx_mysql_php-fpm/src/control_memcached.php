...
// start
$command = "launchctl load -w ~/Library/LaunchAgents/homebrew.mxcl.memcached.plist?" . $id;
$w->result( "$id-start", $command, "Start {$service->name}", "Will run `$command` for you", "icons/{$service->id}.png" );

// stop
$command = "launchctl unload -w ~/Library/LaunchAgents/homebrew.mxcl.memcached.plist?" . $id;
$w->result( "{$service->id}-stop", $command, "Stop {$service->name}", "Will run `$command` for you", "icons/{$service->id}.png" );

// restart
$command = "launchctl unload -w ~/Library/LaunchAgents/homebrew.mxcl.memcached.plist"
		."; launchctl load -w ~/Library/LaunchAgents/homebrew.mxcl.memcached.plist?" . $id;
